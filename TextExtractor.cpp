...
string TextExtractor::Init( const char* pszInput, const char* password)
{
    string text;
	...
	{
		if(error == ePdfError_InvalidPassword && password != 0)
		{
    ...
    for( int i=0; i<nCount; i++ ) 
    {
        ...
        text += this->ExtractText( &document, pPage );
    }

    return text;
}

string TextExtractor::ExtractText( PdfMemDocument* pDocument, PdfPage* pPage )
{
    string text;
                ...
                if( strcmp( pszToken, "Tf" ) == 0 ) 
                {
                    ...
                    if( !pCurFont ) 
                    {
                        fprintf( stderr, "WARNING: Unable to create font for object %i %i R\n",
                         ...
                         strcmp( pszToken, "'" ) == 0 ) 
                {
                    text += AddTextElement( dCurPosX, dCurPosY, pCurFont, stack.top().GetString() );
                    stack.pop();
                }
                ...
                else if( strcmp( pszToken, "TJ" ) == 0 ) 
                {
                    ...
                    for( int i=0; i<static_cast<int>(array.GetSize()); i++ ) 
                    {
                        if( array[i].IsString() || array[i].IsHexString() )
                            text += AddTextElement( dCurPosX, dCurPosY, pCurFont, array[i].GetString() );
                    }
                }
            }
        }
...
string TextExtractor::AddTextElement( double dCurPosX, double dCurPosY,
